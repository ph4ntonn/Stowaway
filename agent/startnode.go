...
func HandleStartNodeConn(connToAdmin *net.Conn, monitor, listenAddr, reConn string, passive bool, nodeid string, proxy, proxyU, proxyP string) {
	payloadBuffChan := make(chan *utils.Payload, 10)
	go HandleStartConn(connToAdmin, payloadBuffChan, monitor, listenAddr, reConn, passive, nodeid, proxy, proxyU, proxyP)
	go HandleDataFromAdmin(connToAdmin, payloadBuffChan, monitor, listenAddr, reConn, passive, nodeid)
	go HandleDataToAdmin(connToAdmin)
}

// HandleStartConn 处理与admin的信道
func HandleStartConn(connToAdmin *net.Conn, payloadBuffChan chan *utils.Payload, monitor, listenAddr, reConn string, passive bool, nodeid string, proxy, proxyU, proxyP string) {
	for {
		AdminData, err := utils.ExtractPayload(*connToAdmin, AgentStatus.AESKey, nodeid, false)
		if err != nil {
			AdminOffline(reConn, monitor, listenAddr, proxy, proxyU, proxyP, passive)
...
func HandleDataFromAdmin(connToAdmin *net.Conn, payloadBuffChan chan *utils.Payload, monitor, listenAddr, reConn string, passive bool, nodeid string) {
	var (
		err          error
